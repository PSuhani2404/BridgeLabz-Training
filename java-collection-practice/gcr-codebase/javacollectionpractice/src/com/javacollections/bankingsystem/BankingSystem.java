package com.javacollections.bankingsystem;
import java.util.*;
public class BankingSystem
{
	    ...
	    public void deposit(String accountNumber, double amount) {
	        // If account is new, we use 0.0 as the base
	        double oldBalance = accounts.getOrDefault(accountNumber, 0.0);
	        double newBalance = oldBalance + amount;
	        
	        // Update the sorted map before updating the main record
	        updateSortedBalance(accountNumber, oldBalance, newBalance, accounts.containsKey(accountNumber));
	    ...
	    public void processWithdrawals() {
	        ...
	        while (!withdrawalQueue.isEmpty()) {
	            ...
	            // Ensure we update the TreeMap to reflect the new lower balance
	            updateSortedBalance(req.accountNumber, currentBalance, newBalance, true);
	    ...
	    private void updateSortedBalance(String acc, double oldBal, double newBal, boolean exists) {
	        // 1. If account existed, remove it from its old balance group
	        if (exists) {
	            List<String> accsAtOldBal = sortedByBalance.get(oldBal);
	            if (accsAtOldBal != null) {
	                accsAtOldBal.remove(acc);
	                if (accsAtOldBal.isEmpty()) {
	                    sortedByBalance.remove(oldBal);
	                }
	            }
	        }
	        // 2. Add to the new balance group (Changed '_' to 'k' for Java compatibility)
	        sortedByBalance.computeIfAbsent(newBal, k -> new ArrayList<>()).add(acc);
	    }

	    // Show current state of all collections
	    public void displayAccounts() {
	        System.out.println("\n=== Banking System Status ===");
	        System.out.println("HashMap (Account Lookups): " + accounts);
	        
	        System.out.println("TreeMap (Richest First):");
	        ...
	        System.out.println("Pending Withdrawal Queue: " + withdrawalQueue);
	        System.out.println("----------------------------------------\n");
	    }
	    ...
	    public static void main(String[] args) {
	        ...
	        bank.requestWithdrawal("ACC003", 80000); // Should fail (insufficient)
